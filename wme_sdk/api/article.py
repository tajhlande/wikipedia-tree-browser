...
class Category:
    ...
    def from_json(data: dict) -> "Category":
        return Category(name=data.get("name"), url=data.get("url"))

    @staticmethod
    def to_json(category: "Category") -> dict:
        return {"name": category.name, "url": category.url}
...
class Redirect:
    ...
    def from_json(data: dict) -> "Redirect":
        return Redirect(name=data.get("name"), url=data.get("url"))

    @staticmethod
    def to_json(redirect: "Redirect") -> dict:
        return {"name": redirect.name, "url": redirect.url}
...
class Template:
    ...
    def from_json(data: dict) -> "Template":
        return Template(name=data.get("name"), url=data.get("url"))

    @staticmethod
    def to_json(template: "Template") -> dict:
        return {"name": template.name, "url": template.url}
...
class Article:
    ...
    def from_json(data: dict) -> "Article":
        return Article(
            ...
            date_previously_modified=Article.parse_date(
                data.get("date_previously_modified")
            ),
            protection=[Protection.from_json(p) for p in data.get("protection", [])],
            version=(
                Version.from_json(data.get("version")) if data.get("version") else None
            ),
            previous_version=(
                PreviousVersion.from_json(data.get("previous_version"))
                if data.get("previous_version")
                else None
            ),
            url=data.get("url"),
            watchers_count=data.get("watchers_count"),
            namespace=(
                Namespace.from_json(data.get("namespace"))
                if data.get("namespace")
                else None
            ),
            in_language=(
                Language.from_json(data.get("in_language"))
                if data.get("in_language")
                else None
            ),
            main_entity=(
                Entity.from_json(data.get("main_entity"))
                if data.get("main_entity")
                else None
            ),
            additional_entities=[
                Entity.from_json(e) for e in data.get("additional_entities", [])
            ],
            ...
            is_part_of=(
                Project.from_json(data.get("is_part_of"))
                if data.get("is_part_of")
                else None
            ),
            article_body=(
                ArticleBody.from_json(data.get("article_body"))
                if data.get("article_body")
                else None
            ),
            license=[License.from_json(l) for l in data.get("license", [])],
            visibility=(
                Visibility.from_json(data.get("visibility"))
    ...
    def to_json(article: "Article") -> dict:
        return {
            ...
            "date_created": (
                article.date_created.strftime("%Y-%m-%dT%H:%M:%SZ")
                if article.date_created
                else None
            ),
            "date_modified": (
                article.date_modified.strftime("%Y-%m-%dT%H:%M:%SZ")
                if article.date_modified
                else None
            ),
            "date_previously_modified": (
                article.date_previously_modified.strftime("%Y-%m-%dT%H:%M:%SZ")
                if article.date_previously_modified
                else None
            ),
            ...
            "previous_version": (
                PreviousVersion.to_json(article.previous_version)
            ...
            "namespace": (
                Namespace.to_json(article.namespace) if article.namespace else None
            ),
            "in_language": (
                Language.to_json(article.in_language) if article.in_language else None
            ),
            "main_entity": (
                Entity.to_json(article.main_entity) if article.main_entity else None
            ),
            "additional_entities": [
                Entity.to_json(e) for e in article.additional_entities
            ],
            ...
            "is_part_of": (
                Project.to_json(article.is_part_of) if article.is_part_of else None
            ),
            "article_body": (
                ArticleBody.to_json(article.article_body)
                if article.article_body
                else None
            ),
            "license": [License.to_json(l) for l in article.license],
            "visibility": (
                Visibility.to_json(article.visibility) if article.visibility else None
            ),
