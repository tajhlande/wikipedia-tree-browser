...
export class LinkManager {
  ...
  private initializeMaterial(): void {
    ...
    // console.log('[LINK] Initialized link material');
  }
  ...
  public createLink(parentNode: ClusterNode, childNode: ClusterNode): Mesh {
    ...
    // console.log(`[LINK] Creating link from node ${parentNode.id} to node ${childNode.id}`);
    ...
    // console.log(`[LINK] Created link from node ${parentNode.id} to node ${childNode.id}`);
  ...
  private createLinkMesh(parentNode: ClusterNode, childNode: ClusterNode): Mesh {
    ...
    // Debug: Log the raw distances and positions
    // console.log(`[LINK] Raw distance between nodes ${parentNode.id} and ${childNode.id}: ${distance}`);
    ...
    // console.log(`[LINK] Direction vector: (${direction.x}, ${direction.y}, ${direction.z})`);
    
    // For now, let's create links that go directly from center to center
    // This will help us verify the basic distance calculation is working
    ...
    this.positionLink(linkMesh, parentPosition, childPosition, direction, finalDistance, parentNode, childNode);
  ...
  private positionLink(
    ...
    // Calculate midpoint for link position (simple center-to-center)
    ...
    // console.log(`[LINK] Positioning link from ${parentNode.id} to ${childNode.id}`);
    // console.log(`[LINK] Parent position: (${parentPosition.x}, ${parentPosition.y}, ${parentPosition.z})`);
    // console.log(`[LINK] Child position: (${childPosition.x}, ${childPosition.y}, ${childPosition.z})`);
   ...
   */
  private updateLink(linkMesh: Mesh, parentNode: ClusterNode, childNode: ClusterNode): void {
    const parentPosition = this.getNodePosition(parentNode);
    const childPosition = this.getNodePosition(childNode);
    const direction = childPosition.subtract(parentPosition);
    const distance = direction.length();
  ...
  private getNodePosition(node: ClusterNode): Vector3 {
    if (node.centroid && node.centroid.length === 3) {
      // Apply the same scaling as used in NodeManager to get the actual node position
      return new Vector3(
