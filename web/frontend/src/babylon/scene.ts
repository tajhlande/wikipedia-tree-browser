...
export function initScene(canvasId: string) {
  try {
    ...
    dirLight.diffuse = new Color3(0.9, 0.9, 0.9);
  ...
  createEffect(() => {
    ...
    if (currentNode) {
      ...
      if (camera && currentNode.centroid) {
        ...
        // Ensure camera controls are still active after positioning
        if (camera.inputs) {
          camera.inputs.attached.mousewheel = true;
          camera.inputs.attached.pointers = true;
          camera.inputs.attached.keyboard = true;
        }
